...
class ProfilController extends Controller {
	...
	public function loadCalendarAction(Request $request) {
		...
		$return_events = array ();
		$events = $this->getDoctrine ()->getManager ()->getRepository ( 'BmdUserBundle:UserAvailability' )->createQueryBuilder ( 'ua' )->where ( 'ua.dateStart >= :startDate' )->andWhere ( 'ua.dateEnd <= :endDate' )->andWhere ( 'ua.accept = 1' )->setParameter ( 'startDate', $startDatetime->format ( 'Y-m-d H:i:s' ) )->setParameter ( 'endDate', $endDatetime->format ( 'Y-m-d H:i:s' ) )->getQuery ()->getResult ();
	...
	private function checkEventAvailable($datestart, $dateend, $userid) {
		$Events = $this->getDoctrine ()->getManager ()->getRepository ( 'BmdUserBundle:UserAvailability' )->createQueryBuilder ( 'e' )->where ( 'e.dateStart <= :startDate AND e.dateEnd >= :startDate AND e.userid = :userId' )->setParameter ( 'startDate', $datestart->format ( 'Y-m-d H:i:s' ) )->setParameter ( 'userId', $userid )->getQuery ()->getResult ();
		
		if (! empty ( $Events ))
			return false;
		$Events = $this->getDoctrine ()->getManager ()->getRepository ( 'BmdUserBundle:UserAvailability' )->createQueryBuilder ( 'e' )->where ( 'e.dateEnd >= :endDate AND e.dateStart <= :endDate AND e.userid = :userId' )->setParameter ( 'endDate', $dateend->format ( 'Y-m-d H:i:s' ) )->setParameter ( 'userId', $userid )->getQuery ()->getResult ();
		if (! empty ( $Events ))
			return false;
		...
		if ($this->container->get ( 'security.context' )->isGranted ( 'IS_AUTHENTICATED_FULLY' )) {
			...
			if ($this->get ( 'request' )->get ( 'name' ) != null) {
				if ($this->get ( 'request' )->get ( 'name' ) == "userFirstName") {
					...
					$usr->setExperience( $this->get ( 'request' )->get ( 'value' ) );
				}
			}
